# scRefBool : R package to quantify gene expression in three levels (low, medium, high)

scRefBool is a single-cell adaptation of RefBool [1] that relies on bulk RNA-seq data and quantifies gene expression into three levels including low, medium and high.
...
#Install scRefBool from GitHub
devtools::install_github("BarlierC/scRefBool")
```
...
Pre-compiled backgrounds to quantify gene expression are available at: https://gitlab.com/C.Barlier/HCellig_backgrounds

*Please note that by using the backgrounds available in this GitLab repository, the gene should be formatted as Symbol for Mouse and Ensembl ID for Human.*
